...
class MapillaryTemporalDataset(Dataset):
    def __init__(self, dataset_dir, train_or_val='train', city = None, n_frames=4, gap=3, dataset_sample=1.0, seed=23):
        super().__init__()
        ...
        feature_paths = list(features_directory.rglob('*.npy'))
        if city:
            df = pd.read_csv("/home/parthpk/XGeoCLIP/geo-clip_new/discrete_frechet-master/video_metrics_phase_2_w_cities.csv")
            city_seq_ids = set(df[df['CITY'] == city]['SEQ_ID'])
            filtered_paths = [path for path in feature_paths if path.name in city_seq_ids]
            feature_paths = filtered_paths
            #print(feature_paths)
            #print(csv)
            #exit()
    ...
    def __getitem__(self, index):
        ...
        if self.train_or_val == 'train':
            ...
            return self.seq_ids[index], features[indices], labels[indices]
        else:
            return self.seq_ids[index], features, labels
...
if __name__ == '__main__':
    ...
    #ds = MapillaryTemporalDataset('/home/c3-0/pr288313/datasets/Mapillary/tensors_video_seq/train', train_or_val='train', n_frames=2, dataset_sample=1.0, seed=23)
    ...
    for seq_id, frames, labels in dl:
        # B, T, C = frames.shape
        # frames_flat = frames.reshape(-1,C)
        # labels_flat = labels.mean(dim=1)
        print(seq_id, frames.shape, labels.shape)
