...
def eval_images(val_dataloader, model, device="cpu"):
    ...
    print()
    acc, avg_distance_error = distance_accuracy(targets, preds, distance_thresholds, gps_gallery)
    for dis in distance_thresholds:
        print(f"Accuracy at {dis} km: {acc[dis]}")
        accuracy_results[f'acc_{dis}_km'] = acc[dis]
    
    error_results = avg_distance_error[dis]
    print()
...
def eval_images_ten_crop(val_dataloader, model, decoder, device="cpu", epoch=-1, mini=False, config=None):
    model.eval()
    decoder.eval()
    preds = []
    targets = []
    ...
    with torch.no_grad():
        ...
        for idx, (seq_id, imgs, pred_labels, labels) in tqdm(enumerate(val_dataloader), desc="Evaluating", total=val_len):
            ...
            if config["use_transformer"]:
                ...
                num_chunks = T//N_Frames + int((T % N_Frames) > 0)
                # imgs_features = []
                # for x in range(num_chunks):
                #     imgs_features.append(model.image_encoder.temp_embed(imgs_by_v[:,x*N_Frames:(x+1)*N_Frames,:]))
                # imgs_features = torch.cat(imgs_features, dim=1)
                imgs_features = model.image_encoder.temp_embed(imgs_by_v)
            imgs_features = imgs_features.view(V*T, -1)
            imgs_features = model.image_encoder.mlp(imgs_features)
            imgs_features = imgs_features.reshape(V,T,-1).mean(dim=0)
            
            pred_features = model.location_encoder(pred_labels)

            imgs_features = F.normalize(imgs_features, dim=1)
            pred_features = F.normalize(pred_features, dim=1)

            # new_features = []
            # for x in range(num_chunks):
            #     new_features.append(decoder(imgs_features[x*N_Frames:(x+1)*N_Frames,:], pred_features[x*N_Frames:(x+1)*N_Frames,:], mode='eval'))
            # new_features = torch.cat(new_features, dim=0)

            new_features = decoder(imgs_features, pred_features, mode='eval')
            ...
            logits_new_gps = temp * (new_features @ gps_features.T)
            outs = torch.argmax(logits_new_gps, dim=-1).detach().cpu().numpy()
            ...
            if config['mode'] == "eval":
               outs_10 = torch.argsort(logits_new_gps, dim=1, descending=True)[:,:10].detach().cpu().numpy()
               folder = f"predictions/{config['exp_name']}/res_1/{epoch}/"
